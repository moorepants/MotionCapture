%Removed bugs from V4:
%       Rear wheel is now round - not oval
%       View rate no longer able to go out of bounds
%       View rate updated when new data is loaded
%       % of each mode is updated upon pressing both/rider/bicycle buttons.
%       FFT buttons are added
...
if get(handles.i_slider,'UserData')==1
    updatepercentagetext(hObject,handles);
end
if get(handles.Li_slider,'UserData')==1
    Lupdatepercentagetext(hObject,handles);
end
guidata(hObject, handles);
...
[handles]=firstplot(hObject,handles, 1);
if get(handles.Li_slider,'UserData')==0 %left view not loaded
    %set playback rate to 1
    set(handles.playBack_edit,'String',num2str(handles.playbackrate(1,5)));
    %set rate
    set(handles.plus_pushbutton,'UserData',handles.playbackrate(:,5));
...
function updatepercentagetext(hObject,handles);
if get(handles.Li_slider,'UserData')==0 %left view not loaded
    %set playback rate to 1
    set(handles.playBack_edit,'String',num2str(handles.playbackrate(1,5)));
    %set rate
...
[handles]=firstplot(hObject,handles, 2);
if get(handles.i_slider,'UserData')==0 %left view not loaded
    %set playback rate to 1
    set(handles.playBack_edit,'String',num2str(handles.playbackrate(1,5)));
    %set rate
    set(handles.plus_pushbutton,'UserData',handles.playbackrate(:,5));
...
function Lupdatepercentagetext(hObject,handles);
if get(handles.i_slider,'UserData')==0 %left view not loaded
    %set playback rate to 1
    set(handles.playBack_edit,'String',num2str(handles.playbackrate(1,5)));
    %set rate
    ...
    function [handles]=firstplot(hObject,handles,nr) %% this function plots the markers and the lines
        ...
        if nr==1
            ...
            elseif (get(handles.bicycle_pushbutton,'UserData') == 1);
                ...
                Pr=[(x(1,16)-x(1,11)) (y(1,16)-y(1,11)) (z(1,16)-z(1,11))];
                R= cross(Sr,Pr);
                Rrnorm = sqrt(R(1)^2+R(2)^2+R(3)^2);
                Rr=R/Rrnorm;
            ...
            %plot bicycle and rider
                ...
                Pr=[(x(1,36)-x(1,31)) (y(1,36)-y(1,31)) (z(1,36)-z(1,31))];
                R= cross(Sr,Pr);
                Rrnorm = sqrt(R(1)^2+R(2)^2+R(3)^2);
                Rr=R/Rrnorm;
...
% --- Executes on button press in Frequency_pushbutton.
function Frequency_pushbutton_Callback(hObject, eventdata, handles)
% hObject    handle to Frequency_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if get(handles.Rcheckbox1,'Value')+get(handles.Rcheckbox2,'Value')+...
   get(handles.Rcheckbox3,'Value')+get(handles.Rcheckbox4,'Value')+...     
   get(handles.Rcheckbox5,'Value')+get(handles.Rcheckbox6,'Value')+...
   get(handles.Rcheckbox7,'Value')+get(handles.Rcheckbox8,'Value')+...
   get(handles.Rcheckbox9,'Value')+get(handles.Rcheckbox10,'Value')==0 ...
    ||get(handles.i_slider,'UserData')==0
    return
end

maxfreq=5                                                                  %max frequency that we will look at
if get(handles.both_pushbutton,'UserData')==1                               %which data to be used?
    A=handles.ABikeRider;
elseif get(handles.bicycle_pushbutton,'UserData')==1
    A=handles.ABike;
else A=handles.ARider;
end

figure
hold on
set(gcf,'Name','FFT of Right Hand Side' )
m=zeros(10,1);
if get(handles.Rcheckbox1,'Value')==1
    [fsa,asa]=freqampl((A(1,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'b')
    m(1)=1;
end
if get(handles.Rcheckbox2,'Value')==1
    [fsa,asa]=freqampl((A(2,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'c')
    m(2)=1;
end
if get(handles.Rcheckbox3,'Value')==1
    [fsa,asa]=freqampl((A(3,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'r')
    m(3)=1;
end
if get(handles.Rcheckbox4,'Value')==1
    [fsa,asa]=freqampl((A(4,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'g')
    m(4)=1;
end
if get(handles.Rcheckbox5,'Value')==1
    [fsa,asa]=freqampl((A(5,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'y')
    m(5)=1;
end
if get(handles.Rcheckbox6,'Value')==1
    [fsa,asa]=freqampl((A(6,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'k')
end
if get(handles.Rcheckbox7,'Value')==1
    [fsa,asa]=freqampl((A(7,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'m')
end
if get(handles.Rcheckbox8,'Value')==1
    [fsa,asa]=freqampl((A(8,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'Color',[0.5 0.5 0.5])
end
if get(handles.Rcheckbox9,'Value')==1
    [fsa,asa]=freqampl((A(9,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'Color',[0.5 0.3 0.7])
end
if get(handles.Rcheckbox10,'Value')==1
    [fsa,asa]=freqampl((A(10,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'Color',[0.7 0.2 1] )
end
%legend(m,'mode 1','mode 2','mode 3','mode 4','mode 5','mode 6','mode 7','mode 8','mode 9','mode 10')


% --- Executes on button press in LFrequency_pushbutton.
function LFrequency_pushbutton_Callback(hObject, eventdata, handles)
% hObject    handle to LFrequency_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if get(handles.Lcheckbox1,'Value')+get(handles.Lcheckbox2,'Value')+...
   get(handles.Lcheckbox3,'Value')+get(handles.Lcheckbox4,'Value')+...     
   get(handles.Lcheckbox5,'Value')+get(handles.Lcheckbox6,'Value')+...
   get(handles.Lcheckbox7,'Value')+get(handles.Lcheckbox8,'Value')+...
   get(handles.Lcheckbox9,'Value')+get(handles.Lcheckbox10,'Value')==0 ...
    ||get(handles.Li_slider,'UserData')==0
    return
end

maxfreq=5                                                                  %max frequency that we will look at
if get(handles.both_pushbutton,'UserData')==1                               %which data to be used?
    A=handles.LABikeRider;
elseif get(handles.bicycle_pushbutton,'UserData')==1
    A=handles.LABike;
else A=handles.LARider;
end
...
set(gcf,'Name','FFT of Right Hand Side' )
m=zeros(10,1);
if get(handles.Lcheckbox1,'Value')==1
    [fsa,asa]=freqampl((A(1,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'b')
    m(1)=1;
end
if get(handles.Lcheckbox2,'Value')==1
    [fsa,asa]=freqampl((A(2,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'c')
    m(2)=1;
end
if get(handles.Lcheckbox3,'Value')==1
    [fsa,asa]=freqampl((A(3,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'r')
    m(3)=1;
end
if get(handles.Lcheckbox4,'Value')==1
    [fsa,asa]=freqampl((A(4,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'g')
    m(4)=1;
end
if get(handles.Lcheckbox5,'Value')==1
    [fsa,asa]=freqampl((A(5,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'y')
    m(5)=1;
end
if get(handles.Lcheckbox6,'Value')==1
    [fsa,asa]=freqampl((A(6,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'k')
end
if get(handles.Lcheckbox7,'Value')==1
    [fsa,asa]=freqampl((A(7,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'m')
end
if get(handles.Lcheckbox8,'Value')==1
    [fsa,asa]=freqampl((A(8,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'Color',[0.5 0.5 0.5])
end
if get(handles.Lcheckbox9,'Value')==1
    [fsa,asa]=freqampl((A(9,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'Color',[0.5 0.3 0.7])
end
if get(handles.Lcheckbox10,'Value')==1
    [fsa,asa]=freqampl((A(10,:)'),1/100);
    loc = (fsa<maxfreq);
    plot(fsa(loc),asa(loc),'Color',[0.7 0.2 1] )
end
...
function [freq,ampl] = freqampl(y,tsample)

Fs = 1/tsample;
n = length(y);
y = y -sum(y)./n;                                                           %remove the average
Y = fft(y)/n;
